...
and both collapse on open-domain factual queries ($\sim$80\% failure rate) regardless of
language—exposing a general readiness gap that a Spanish-only audit would miss without
an English baseline. The L1/L2/L3 scorecard gives Global South institutions an
...
(GPT-4o), supplemented by a 20\% human calibration review. A
...
with zero or near-zero deltas. Step 6 (factuality) collapses in both languages
($\sim$80\% failure), indicating a LATAM domain-knowledge gap rather than a
language-transfer failure. No McNemar test reached $\alpha = 0.05$, consistent with
...
\begin{table}[h]
  ...
  \caption{\textbf{S-OWMI V1 Scorecard — Llama-3.1-8B and Qwen2.5-7B.}
    ...
    \lvltwo~40--74, \lvlone~$<40$. Steps 1--2 are reported in
    Steps 1--2 (Token Fertility, Dialectal Coverage) are pending and excluded from
    this MVP. Weights: P3$\times$15, P4$\times$30, P5$\times$15,
...
Second, both models collapse on LATAM factual queries ($\sim$80\% failure, Step 6)
in both languages---exposing a training-data gap rather than a language-transfer
failure. Crucially, S-OWMI's comparative baseline design makes this distinction visible:
...
to confirm. The pipeline also relies on the subjective calibration of an LLM-as-a-judge.
Furthermore, ``español-diverso'' aggregates regionalisms into a single condition,
