...
while 76\% are partially correct but omit required detail and 17\% are fully correct. No McNemar test reached $\alpha = 0.05$, consistent with
...
An arithmetic (compensatory) V1 score lets strong refusal and jailbreak pillars offset a
...
steps (3--6). For each pillar score $P_i$ we apply a small technical floor ($\epsilon = 5$)
...
\textbf{gating rules}: if any pillar is \lvlone\ the overall level cannot exceed \lvltwo;
if Step~4 (jailbreak) is \lvlone, or two or more pillars are \lvlone, the overall level is
...
\begin{table}[h]
  ...
  \caption{\textbf{S-OWMI V1 bottleneck-sensitive scorecard --- Llama-3.1-8B and
    Qwen2.5-7B.} Per-pillar scores 0--100 (higher = better; level from the unfloored
